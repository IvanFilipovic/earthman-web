...
import { defineComponent, mergeProps, unref, withCtx, createVNode, createBlock, createCommentVNode, toDisplayString, openBlock, resolveDynamicComponent, Transition, useSSRContext, ref, computed, withAsyncContext, watch, createTextVNode, Fragment, renderList } from "vue";
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(variantGroups).length) {
        ...
        ssrRenderList(unref(variantGroups), (vg, idx) => {
          _push(`<button class="${ssrRenderClass([unref(activeColorIndex) === idx ? "ring-2 ring-text_color" : "", "w-8 h-8 border"])}" style="${ssrRenderStyle({ backgroundColor: swatchBg(vg.color?.image) })}"></button>`);
        });
        _push(`<!--]--></div>`);
        if (unref(activeColorName)) {
          _push(`<p class="mt-1 text-sm text-text_color">Color: ${ssrInterpolate(unref(activeColorName))}</p>`);
...
We accept returns within 14 days. Sale items can only be refunded as store credit.`,
      ...
      if (unref(product)?.discount) {
        _push(`<div class="space-x-2"><span class="text-base font-semibold text-text_color">€${ssrInterpolate(unref(product)?.discount_price)}</span><span class="line-through text-text_color">€${ssrInterpolate(unref(product)?.price)}</span></div>`);
      } else {
        _push(`<div class="text-base font-semibold text-text_color">€${ssrInterpolate(unref(product)?.price)}</div>`);
      }
                              ...
                              if (unref(activeSizes).length) {
                                ...
                                ssrRenderList(unref(activeSizes), (s) => {
                                  _push5(`<button class="${ssrRenderClass([[
                                    s.available ? "cursor-pointer" : "opacity-40 cursor-not-allowed",
                                    unref(selectedVariantSlug) === s.slug ? "border-text_color" : "border-project_gray/50"
                                  ], "border px-3 py-2 text-sm w-fit"])}"${_scopeId4}>${ssrInterpolate(s.size?.name)}</button>`);
                                });
                                _push5(`<!--]--></div></div>`);
                              } else {
                                _push5(`<!---->`);
...
//# sourceMappingURL=_...product_-BwJkuA5H.js.map
