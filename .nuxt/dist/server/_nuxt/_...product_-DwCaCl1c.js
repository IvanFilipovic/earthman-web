...
import { i as useNuxtApp, c as useRoute, b as useRouter } from "../server.mjs";
import "@iconify/vue";
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(variantGroups).length) {
        ...
        ssrRenderList(unref(variantGroups), (vg, idx) => {
          _push(`<button class="${ssrRenderClass([unref(activeColorIndex) === idx ? "ring-2 ring-text_color" : "", "w-8 h-8 border relative"])}"${ssrRenderAttr("title", vg.color?.name)} style="${ssrRenderStyle({ backgroundImage: `url(${vg.avatar_image})` })}"><span class="sr-only">${ssrInterpolate(vg.color?.name)}</span></button>`);
        });
        _push(`<!--]--></div></div>`);
      } else {
        _push(`<!---->`);
      ...
      _push(`<div class="mt-6 space-y-3"><button class="primary-btn w-full py-3"${ssrIncludeBooleanAttr(!unref(selectedVariantSlug)) ? " disabled" : ""}> ADD TO CART </button></div><ul class="mt-5 space-y-2 text-sm text-text_color/80"><li>✓ Free delivery over €250,-</li><li>✓ Easy returns within 14 days</li></ul></div></aside></div>`);
...
//# sourceMappingURL=_...product_-DwCaCl1c.js.map
