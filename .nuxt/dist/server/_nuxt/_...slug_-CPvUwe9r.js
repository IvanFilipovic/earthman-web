...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const getStandardImageAttrs = (url) => {
      return getResponsiveImages(
        url,
        [400, 600, 800, 1200, 1600],
        "(max-width: 767px) 100vw, 33vw"
      ...
      ssrRenderList(unref(sections), (section, idx) => {
        ...
        if (section.layout === "hero") {
          ...
          ssrRenderList(section.items, (item) => {
            _push(`<article class="group cursor-pointer relative h-[80vh] overflow-hidden w-full md:w-2/3 mx-auto"><img${ssrRenderAttrs(mergeProps({ ref_for: true }, getHeroImageAttrs(item.currentImage || item.colors?.[0]?.avatar_image), {
              alt: item.alt_text || item.name,
              class: "w-full h-full object-cover transition-transform duration-700 group-hover:scale-105",
              loading: "lazy"
            }))}><div class="absolute inset-0"></div><div class="absolute bottom-0 left-0 right-0 p-4 md:p-6 text-text_color bg-background_color/85 w-full"><div class="hero-content"><p class="text-xs tracking-widest uppercase mb-1 opacity-80">${ssrInterpolate(item.category)}</p><h2 class="hero-title text-xl md:text-2xl font-bold mb-1 uppercase tracking-tight">${ssrInterpolate(item.name)}</h2><div class="flex items-center gap-3 mb-2">`);
            ...
            ssrRenderList(item.colors?.slice(0, 4), (color, colorIdx) => {
              _push(`<div class="relative"><img${ssrRenderAttrs(mergeProps({ ref_for: true }, getSwatchImageAttrs(color.avatar_image), {
                alt: color.color,
          ...
          ssrRenderList(section.items, (item) => {
            _push(`<article class="group cursor-pointer relative h-[100vh] overflow-hidden w-full"><img${ssrRenderAttrs(mergeProps({ ref_for: true }, getTripleImageAttrs(item.currentImage || item.colors?.[0]?.avatar_image), {
              alt: item.alt_text || item.name,
              class: "w-full h-full object-cover transition-transform duration-700 group-hover:scale-105",
              loading: "lazy"
...
//# sourceMappingURL=_...slug_-CPvUwe9r.js.map
