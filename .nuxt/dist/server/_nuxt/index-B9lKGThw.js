...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const getMainImageAttrs = (url) => {
      return getResponsiveImages(
        url,
        [300, 400, 600, 800],
        // Widths for srcset
        "(max-width: 767px) 50vw, 25vw"
        // Mobile: 2 cols (50%), Desktop: 4 cols (25%)
      );
    };
    const getMobileSliderImageAttrs = (url) => {
      return getResponsiveImages(
        url,
        [300, 400, 600],
        // Mobile-focused widths
        "50vw"
        // 2 columns on mobile = 50% viewport width
      );
    };
    const getSwatchImageAttrs = (url) => {
      return getResponsiveImages(
        url,
        [50, 100],
        // Just 1x and 2x for small thumbnails
        "32px"
        // Fixed size (w-8/h-8)
      );
    };
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "grid grid-cols-2 md:grid-cols-4 overflow-y-auto pt-8 gap-x-4 md:gap-x-8 lg:gap-x-24" }, _attrs))} data-v-28eb2f71><!--[-->`);
      ssrRenderList(__props.products, (item) => {
        _push(`<div class="col-span-1 cursor-pointer pb-10" data-v-28eb2f71><div class="bg-background_color overflow-hidden object-fill" data-v-28eb2f71><div class="w-full" data-v-28eb2f71><div class="relative bg-background_color" data-v-28eb2f71><img${ssrRenderAttrs(mergeProps({ ref_for: true }, getMainImageAttrs(getMainImage(item)), {
          alt: item.alt_text || item.name,
          class: "hidden md:block max-w-[100%] h-auto object-cover transition-opacity duration-200 border border-text_color/30",
          loading: "lazy"
        }))} data-v-28eb2f71>`);
        if (item.colors && item.colors.length > 0) {
          _push(`<div class="block md:hidden" data-v-28eb2f71><div class="keen-slider" data-v-28eb2f71><!--[-->`);
          ssrRenderList(item.colors, (color, i) => {
            _push(`<div class="keen-slider__slide" data-v-28eb2f71><img${ssrRenderAttrs(mergeProps({ ref_for: true }, getMobileSliderImageAttrs(color.avatar_image), {
              alt: color.color,
        ...
        _push(`<button type="button" class="${ssrRenderClass([{ "opacity-100": unref(hoveredSlug) === item.slug }, "absolute top-4 left-4 bg-text_color text-background_color tracking-wider text-xs font-light w-auto px-2 mx-auto opacity-0 transition-opacity duration-200"])}" style="${ssrRenderStyle(unref(hoveredSlug) === item.slug ? null : { display: "none" })}" data-v-28eb2f71> VIEW </button></div><div class="py-3 text-left" data-v-28eb2f71><p class="text-xs tracking-wider font-extralight" data-v-28eb2f71>${ssrInterpolate(item.category)}</p><h3 class="uppercase tracking-wider text-[13px] md:text-sm font-normal truncate py-1" data-v-28eb2f71>${ssrInterpolate(item.name)}</h3>`);
        if (item.colors && item.colors.length > 0) {
          _push(`<div class="mt-1 flex items-center gap-2" data-v-28eb2f71><!--[-->`);
          ssrRenderList(item.colors, (color, colorIdx) => {
            _push(`<div class="relative group" data-v-28eb2f71><img${ssrRenderAttrs(mergeProps({ ref_for: true }, getSwatchImageAttrs(color.avatar_image), {
              alt: color.color,
              class: "w-7 h-7 md:h-8 md:w-8 object-cover border border-project_black/30 cursor-pointer",
              loading: "lazy"
            }))} data-v-28eb2f71><div class="${ssrRenderClass([isColorHovered(item, colorIdx) ? "opacity-100 bg-text_color" : "opacity-0 bg-text_color", "absolute inset-0 pointer-events-none transition-opacity duration-200 w-8 h-8 bg-text_color"])}" data-v-28eb2f71>`);
        ...
        if (item?.discount) {
          _push(`<div class="space-x-2" data-v-28eb2f71><span class="mt-1 text-xs font-light tracking-widest text-error_text_color" data-v-28eb2f71>€${ssrInterpolate(formatPrice(item.discount_price))}</span><span id="price" class="mt-1 text-xs font-light tracking-widest text-text_color" data-v-28eb2f71>€${ssrInterpolate(formatPrice(item.price))}</span></div>`);
        } else {
          _push(`<p class="mt-1 text-xs font-light tracking-widest text-text_color" data-v-28eb2f71>€${ssrInterpolate(formatPrice(item.price))}</p>`);
        }
...
const __nuxt_component_3 = /* @__PURE__ */ _export_sfc(_sfc_main$1, [["__scopeId", "data-v-28eb2f71"]]);
const PAGE_SIZE = 12;
...
//# sourceMappingURL=index-B9lKGThw.js.map
