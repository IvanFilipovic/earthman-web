...
<script setup lang="ts">
  ...
  interface Filters {
    gender: string[]
    collections: string[]
    categories: string[] // Changed from number[] to string[]
    sizes: string[]
    colors: string[]
  }
  ...
  const filters = ref<Filters>({
    gender: [],
    collections: [],
    categories: [], // Now stores category names
    sizes: [],
    colors: []
  })
  ...
  const facets = computed(() => facetsStore.facets)
  ...
  function initializeFiltersFromQuery(): void {
    ...
    if (query.category) {
      filters.value.categories = Array.isArray(query.category)
        ? query.category.map(String) // Changed from Number to String
  ...
  function buildQueryParams(): URLSearchParams {
    ...
    filters.value.categories.forEach(name => params.append('category', name)) // Now sends names
  ...
  function updateRouteQuery(): void {
    ...
    if (filters.value.categories.length > 0) {
      query.category = filters.value.categories // Now stores names in URL
    }
